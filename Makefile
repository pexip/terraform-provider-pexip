# Default values for provider, company, and domain. Can be overridden from environment.
PROVIDER     ?= pexip
COMPANY      ?= pexip
DOMAIN       ?= pexip.com
...
# Git metadata for embedding in the binary
...
# Variables for linting, can be overridden.
GO_LINT_HEAD ?=
GO_ENV_VARS  ?=

# Phony targets do not represent files.
.PHONY: all prepare lint build package install test testacc clean manifest version
...
testacc: prepare
	TF_ACC=true go test -v -tags integration -coverprofile=$(BUILD_DIR)/cover.out ./...

# Clean up build artifacts and installed provider
clean:
	@echo "Cleaning up..."
	@rm -rf $(BUILD_DIR)
	@rm -rf ~/.terraform.d/plugins/$(DOMAIN)/$(COMPANY)/$(PROVIDER)/$(VERSION_NO_V)
